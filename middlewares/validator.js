...
exports.validateMovie = [
  ...
  check("tags")
    .isArray({ min: 1 })
    .withMessage("Movie tags must be an array of strings")
    .custom((tags) => {
      for (let tag of tags) {
        if (typeof tag !== "string")
          throw Error("Movie tags must be an array of strings");
      }
      return true;
    }),
  check("cast")
    ...
    .custom((cast) => {
      for (let c of cast) {
        ...
        if (typeof c.leadActor !== "boolean")
          throw Error(
            "Only accepted boolean values inside leadActor inside cast"
          );
        return true;
      }
    }),
