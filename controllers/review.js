...
exports.addReview = async (req, res) => {
  ...
  if (!isValidObjectId(userId)) return sendError(res, "Invalid Movie");
...
exports.removeReview = async (req, res) => {
  ...
  const movie = await Movie.findById(review.parentMovies).select("reviews");
...
exports.getReviewsByMovie = async (req, res) => {
  ...
  const reviews = movie.reviews.map((r) => {
    const { owner, content, rating, id: reviewId } = r;
  ...
  res.json({ movie: { title: movie.title, reviews: movie.reviews } });
};
